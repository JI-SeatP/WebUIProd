...
export function CNCLabel({ data }: CNCLabelProps) {
  const now = new Date();
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Product / Item — NO_INVENTAIRE bold, then INVENTAIRE_S */}
        <div style={{ marginTop: "6px" }}>
          <div style={{ fontSize: TITLE_SZ }}>Produit / Item #:</div>
          <div style={{ fontSize: VAL.NO_INVENTAIRE, fontWeight: 700, lineHeight: 1.1 }}>{data.NO_INVENTAIRE}</div>
          <div style={{
            fontSize: VAL.INVENTAIRE_S, lineHeight: 1.1,
            height: "30.8pt", overflow: "hidden",
            display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical",
          }}>{data.INVENTAIRE_S ?? ""}</div>
        </div>
        ...
        {/* Panel — at bottom of block, 14pt, not bold */}
        <div style={{ marginTop: "6px" }}>
          <div style={{ fontSize: TITLE_SZ }}>Panneau / Panel #:</div>
          <div style={{ fontSize: VAL.Panneau, lineHeight: 1.1 }}>{data.Panneau}</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        {/* Boxes — row 2, same height via grid */}
